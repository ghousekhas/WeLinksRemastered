...
export default class AddressList extends React.Component {
  ...
  onBackPress = () => {
   ...
   this.props.route.params.navigator.navigate('HomeStack')
  ...
  retrieveAddresses = () => {
    ...
    Axios.get(Config.api_url + 'php?action=getUserAddresses&' + data)
      .then((response) => {
        console.log('response', response.data.addresses.length);
  ...
  renderSavedAddress = ({ item, index }) => {
    ...
    return <HomeAddress item={{ ...item, type: 'pin' }} style={styles.horiz} deletae={this.state.myAddresses} route={{
      params: {
        next: next,
