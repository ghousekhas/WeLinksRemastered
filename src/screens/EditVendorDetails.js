...
export default function EditVendorDetails({ route, navigation }) {
    const [presentDetails, setPresentDetails] = useState(route.params.VendorProfileDetails);
 ...
 //   console.log("prp" + presentDetails.address)
    ...
    const updateDetails = () => {
       ...
       // if (aadharFile !== null)
           ...
           if(aadharFile != undefined && aadharFile!= null && aadharFile.name != null)
       { 
    ...
    return (
        <View style={{ ...StyleSheet.absoluteFill, backgroundColor: 'white' }}>
            ...
            <View style={{ marginTop: dimen.height / 20, height: dimen.height * 0.77 }}>
                <ScrollView >
                    <TextBox defaultValue={presentDetails.company_name} title="NAME OF YOUR COMPANY" hint="Enter your company's name" changeText={companyName} />
                    <TextBox defaultValue={presentDetails.company_email_id} title="COMPANY EMAIL ADDRESS" hint="Enter company's E-mail address" changeText={companyEmail} />
                    <TextBox defaultValue={presentDetails.gstin} title="COMPANY GST NUMBER" hint="Enter company's GST number" changeText={companyGstNumber} />
                    <UploadButton title='CHANGE GST CERTIFICATE' browseresult={fileselect} fileSetter={setGSTFile} />
                    <UploadButton title='CHANGE ADDRESS' buttonTitle='Map' setAddress={setAddress} actualUser={actualUser} />
                    <UploadButton title='CHANGE AADHAR/VERIFICATION' browseresult={fileselect} fileSetter={setAadharFile} />
                </ScrollView>
            </View>
...
const UploadButton = ({ hint, title, browseresult, fileSetter, actualUser, buttonTitle = 'Browse', setAddress, address }) => {
