...
export default function App() {
  ...
  if (use == AuthConstants.phone_verified ) 
    ...
    if (apendingAction > 0 && apendingActionItem != null && true /*add scrap condition*/) {
      ...
      return <ScrapPickedConfirmation route={{ params: { item: apendingActionItem, refreshCallback: ()=>{ authContext.sync();}, actualUser: userDetails, cardDetail: pendingActionItem, tag: 'User', user: user } }} />
    }
    else if (apendingAction > 0 && apendingActionItem != null && true /*corporate scrap condition*/) {
      return null;
    }
